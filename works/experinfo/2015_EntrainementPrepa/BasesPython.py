...
"""
on liste les attribut après le constructeur __init__
on définit les méthodes propres à la classe
"""
...
p=Pile1() #il n'y a pas d'arguments à donner pour créer une Pile1
for i in range(1,11):
    p.push(i)
while not p.empty():
    print(p.pop(), end=' - ') #le séparateur end='-' remplace la valeur par défault end='\n' qui passe à la ligne
...
class Pile2:
    ...
    def push(self,x):
        c=Cell(x)
        c.next=self.lst # on définit le successeur de la cellule c comme la pile
        self.lst = c # on définit la nouvelle pile comme commençant à c
...
class Liste_d:
    ...
    def delend(self):
        if self.len is 0:
            ...
            self.last, self.last.next = self.last.prec, None
    
    def delbeg(self):
        if self.len is 0:
            raise ValueError("empty")
        else:
            self.first, self.first.prec = self.first.next, None
...
vrai1 = issubclass(AgentSpecial, Personne) and issubclass(Personne, object)   #objet : classe grand-mere de toutes celles que l'on peut creer et qui definit nottament les methodes __init__ etc.
...
class DictionnaireOrdonne:
    ...
    def __repr__(self):
        """representation de l'objet, renvoye lorsqu'on saisit le dic dans l'interpreteur ou applique repr(dic) """
        chaine = "{"
...
def memoise(func):
    cache={}
    def wrapper(*args):           # * est l'opérateur de débalage qui s'applique à un tuple (longueur indeterminee) de plusieurs arguments et renvoie le tuple des arguments au lieu d'un seul argument tuple
...
'''
UN BEL EXEMPLE DE CONSTRUCTEUR DE DECORATEURS : 
...
def controler_temps(nb_secs):
    """renvoie un decorateur destine a controler que le temps mis par la fonction qu'il decorera est inferieur a nb_secs"""
    def decorateur(fonction_a_executer):
        """ce decorateur va remplacer fonction_a_executer par fonction_modifiee"""
...
'''CECI EST UN SUPERBE EXEMPLE DE CONSTRUCTEUR DE DECORATEURS PRENANT UN NB VAR DE PARAM'''

def controler_types(*t_args, **t_kwargs): # param non nommés *t_args et nommés **t_kwargs du décorateur
    def decorateur(fonction_a_executer): # décorateur prend fonction_a_executer et renvoie fonction_modifiee
        def fonction_modifiee(*args, **kwargs): #param non nommés et nommés de fonction_modifiee
            if len(args) != len(t_args):
                raise TypeError("il y a {} arguments au lieu de {}".format(len(args), len(t_args)))
            for i, arg in enumerate(args): # args est un tuple
                if t_args[i] is not type(args[i]):
                    raise TypeError("L'arg. libre num. {} a type {} au lieu de {}".format(i, type(args[i]), t_args[i]))
            for cle in kwargs: # kwargs est un dictionnaire
        ...
        return fonction_modifiee # le décorateur renvoie la fonction modifiée
    return decorateur # on renvoie le decorateur construit (qui dépend des paramètres donnés au constructeur)
...
#sorted 
#trie un iterable (ici liste ) avec la fonction d'ordre ici key : ordre lexico sur les minuscules
...
except MonException as e: # as comme dans import numpy as np
